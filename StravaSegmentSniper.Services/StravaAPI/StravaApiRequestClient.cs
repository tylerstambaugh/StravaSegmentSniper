...
{
    internal class StravaApiRequestClient : IStravaApiRequestClient
    {
        private readonly IStravaApiRequestClientConfiguration _configuration;
        ...
        public StravaApiRequestClient(IStravaApiRequestClientConfiguration configuration): this(null, configuration) { }


        internal StravaApiRequestClient(HttpMessageHandler httpMessageHandler, IStravaApiRequestClientConfiguration configuration)
        {
        ...
        private async Task GetRefreshToken()
        {
            using (var httpClient = new HttpClient(HttpClientHandler))
            {
                httpClient.BaseAddress = new Uri(_configuration.AccountBaseUrl);
                httpClient.DefaultRequestHeaders.Accept.Clear();
                httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));

                var query = $"refresh_token={_configuration.RefreshToken}&client_id={_configuration.ClientId}&client_secret={_configuration.ClientSecret}&grant_type=refresh_token";
